...
La extensión Dev Containers de VS Code es la forma estándar de operar el laboratorio en este track, pero no es la única posible. Una vez visto que el contenedor se levanta con `docker run` y el workspace es un bind mount, dos formas adicionales de trabajar quedan disponibles sin que el modelo del lab cambie en nada.

La primera es **editor en el host más `docker exec`**: se edita el repo desde cualquier editor del host —Vim, Emacs, Helix, Neovim, Sublime, otra IDE— aprovechando que los archivos están físicamente ahí, y para compilar o ejecutar se abre una shell dentro del contenedor con `docker exec -it <contenedor> bash`. Este flujo no requiere ninguna extensión ni plugin específico; sólo Docker corriendo y el contenedor levantado como muestra este capítulo.

La segunda es **cliente remoto propio del editor**: editores como Neovim, Helix, Emacs o las IDEs de JetBrains tienen modos para conectarse a un proceso de servidor que corre dentro del contenedor —vía SSH, vía un protocolo propio, o vía un plugin del estilo *remote container*. Cada uno con su configuración específica que excede a este nivel; la primitiva subyacente sigue siendo la misma que VS Code automatiza.

Ninguna de las dos formas se documenta paso a paso acá, porque cada editor tiene su propio camino y los detalles envejecen rápido. Lo que importa para `L0` es que la elección de editor es ortogonal al laboratorio: el contenedor, la imagen, el bind mount y la toolchain son los mismos objetos en cualquier flujo, y todas las observaciones de los seis capítulos se sostienen sin cambios.
