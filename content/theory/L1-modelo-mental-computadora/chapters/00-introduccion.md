...
Antes de cualquier tecnicismo, una imagen cotidiana alcanza para nombrar las dos ideas que el nivel va a desplegar con cuidado: **algoritmo** y **máquina universal**.

Una receta de cocina, escrita en un papel o guardada en un archivo de texto, es **un dato**. Como hoja, no hace nada: es tinta sobre papel o caracteres en disco. Si esa misma hoja la lee un cocinero, o la procesa un artefacto que sabe interpretar instrucciones de cocina, el contenido del papel deja de ser solamente texto. Se vuelve una **receta en ejecución**: pasos que se aplican uno tras otro, ingredientes que cambian de estado, un plato que termina existiendo. La materia escrita en la hoja sigue siendo la misma; el efecto es radicalmente distinto según exista o no algo capaz de leerla y aplicarla.
...
El ISA del nivel no es una versión reducida de ningún ISA real ni un híbrido de varios. Está diseñado solamente para que las trazas sean legibles; mapearlo término a término contra x86 o RISC-V no funciona, y eso es deliberado.
